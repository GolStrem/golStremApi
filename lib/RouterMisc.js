  ...
  return async (req, res, next) => {
    ...
    if (type == 'fiche') {
      ...
      if (fiche.idOwner == null && fiche.idUnivers != null){
        const exist = await db.exist('select 1 from userUnivers where idUnivers = ? and idUser = ? and state >= ?', fiche.idUnivers, session.getUserId(), value)
    ...
    if (type == 'module') {
      ...
      const exist = await db.exist(qryStruct, idModule, session.getUserId(), session.getUserId(), session.getUserId(), value ?? 2)
      if(!exist) { return res.status(403).send("no authorization") }
